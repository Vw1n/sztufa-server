...
- **球员管理与停赛重置** - 球员数据管理，支持基于比赛场次自动判定并恢复被禁赛（`suspended`）球员的状态为可用（`active`），并在 CreatePlayerDto 接口中支持传入可用状态及红黄牌以兼容批量录入。
- **赛季归档与管理 [NEW]** - 独立的赛季系统。支持赛季多端切换，一键归档旧赛季并开启新赛季，触发事务自动重置所有球员在新赛季的黄红牌数与可用状态。
- **荣誉与助攻数据统计 [NEW]** - 比分录入支持助攻球员（`assist`）绑定，比赛基本信息中支持评选该场最佳球员（MVP），同时支持球员个人跨赛季生涯统计聚合 API。
- **比赛管理** - 比赛日程、结果记录和统计
- **数据导入** - 支持从 JSON 文件批量导入数据
- **赛事关键事件流** - 支持进球（普通/点球/乌龙）、换人（双下拉列表）、红黄牌等事件的时序数据存取
- **操作审计日志 [NEW]** - 记录管理员关键行为，已支持细粒度属性级数据修改差异比对（比分、球员资料、权限变化等），并支持短时连续相同操作的智能合并聚合及描述折叠压缩，剔除登录等无修改的冗余操作记录。
